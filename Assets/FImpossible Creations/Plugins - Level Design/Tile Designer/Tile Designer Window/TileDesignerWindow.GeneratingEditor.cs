...
namespace FIMSpace.Generating
{
    public partial class TileDesignerWindow
    {
        ...
        private void GUIEditor()
        {
            ...
            { _techInfoContent.text = " Loft?"; if (GUILayout.Button(_techInfoContent, EditorStyles.label, GUILayout.Width(48), GUILayout.Height(16))) { EditorUtility.DisplayDialog("Loft Technique", "Loft technique is dedicated for generating walls,roofs,curved walls,pavements meshes, but you can also find some additional uses for it.", "Ok"); } }
            else if (EditedTileSetup.GenTechnique == EMeshGenerator.Lathe) { _techInfoContent.text = " Lathe?"; if (GUILayout.Button(_techInfoContent, EditorStyles.label, GUILayout.Width(58), GUILayout.Height(16))) { EditorUtility.DisplayDialog("Lathe Technique", "Lathe technique is dedicated for generating pillars, columns, rungs or other cylindrical elements.", "Ok"); } }
            else if (EditedTileSetup.GenTechnique == EMeshGenerator.Extrude) { _techInfoContent.text = " Extrude?"; if (GUILayout.Button(_techInfoContent, EditorStyles.label, GUILayout.Width(69), GUILayout.Height(16))) { EditorUtility.DisplayDialog("Extrude Technique", "Extrude technique is dedicated for generating cut-shapes, for example to cut doorway shape (in the combine mode) in wall generated with Loft technique.", "Ok"); } }
            ...
            if (GenTechnique == EMeshGenerator.Loft)
                ...
                GUI_ExtrudeTopPanel();
            else if (GenTechnique == EMeshGenerator.Sweep_NotImplemented)
                GUI_SweepTopPanel();
            ...
            if (!_foldout_bigPreview) if (allowDraw)
                {
                    ...
                    else if (GenTechnique == EMeshGenerator.Sweep_NotImplemented) DrawSweepEditor(editorRect, displayRect);
                    else if (GenTechnique == EMeshGenerator.CustomMesh) DrawCustomMeshEditor(editorRect, displayRect);
            ...
            //if (tileMeshPreviewEditor != null) tileMeshPreviewEditor.OnInteractivePreviewGUI(prevRect, EditorStyles.textArea);
