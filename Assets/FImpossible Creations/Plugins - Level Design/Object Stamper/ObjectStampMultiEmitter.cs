                    ...
                    if (setParams.LimitMode == OStamperMultiSet.MultiStamperSetParameters.ECountLimit.OneLimitForThisSet)
                    {
                        ...
                        for (int sp = 0; sp < targetSpawnCount; sp++)
                        {
                            ...
                            for (int r = 0; r < MaxRetryAttempts + 1; r++) // When spawned successfully then loop breaks
                            {
                                ...
                                spawningInfo = stamperSet.Emit();
                                GameObject spawned = SpawnEmitPrefab(stamperSet);
                                if (spawned) { count++; break; } // Succesfully spawned
                            }
                        }
                    }
                        ...
                        for (int prId = 0; prId < MultiSet.PrefabsSets[internalSelected].Prefabs.Count; prId++)
                        {
                            ...
                            for (int spawnEmissions = 0; spawnEmissions < targetSpawnCount; spawnEmissions++)
                            {
                                ...
                                for (int r = 0; r < MaxRetryAttempts + 1; r++) // When spawned successfully then loop breaks
                                {
                                    ...
                                    spawningInfo = stamperSet.GenerateInfoForPrefab(MultiSet.PrefabsSets[internalSelected].Prefabs[prId]);
    ...
    public class ObjectsStampMultiEmitterEditor : ObjectsStampEmitterBaseEditor
    {
        ...
        protected override void DrawProperties()
        {
            ...
            if (GUILayout.Button("+", GUILayout.Width(24)))
                Get.Areas.Add(new ObjectStampMultiEmitter.SpawnArea("Area " + Get.Areas.Count.ToString()));
            ...
            if (Get.Selected > -1)
            {
                ...
                if (area != null)
                {
                    ...
                    if (GUILayout.Button(new GUIContent(FGUI_Resources.Tex_Remove), FGUI_Resources.ButtonStyle, new GUILayoutOption[2] { GUILayout.Width(24), GUILayout.Height(24) })) { Get.Areas.RemoveAt(Get.Selected); EditorUtility.SetDirty(Get); }
                    ...
                    if (area.Shape == ObjectStampMultiEmitter.SpawnArea.EShape.Points)
                    {
                        ...
                        //GUILayout.FlexibleSpace();
                        if (GUILayout.Button("+", GUILayout.Width(24))) { area.Points.Add(new ObjectStampMultiEmitter.EmitPoint(Vector3.forward)); }
                        EditorGUILayout.EndHorizontal();
                    ...
                    GUILayout.FlexibleSpace();
                    if (GUILayout.Button("+", GUILayout.Width(24))) { area.Sets.Add(-1); area.Multiply.Add(1f); }
                    EditorGUILayout.EndHorizontal();
