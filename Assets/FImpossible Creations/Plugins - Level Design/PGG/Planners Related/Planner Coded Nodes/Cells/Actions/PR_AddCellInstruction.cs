...
namespace FIMSpace.Generating.Planning.PlannerNodes.Cells.Actions
{
    ...
    {
        public override string GetDisplayName(float maxWidth = 120) { return "Add Cell Instruction"; }
        ...
        void AddInstruction(FieldCell cell, PlannerResult reslt)
        {
            ...
            if (Operation == ECellOperation.AddCellInstructionID)
            {
                ...
                {
                    instr.WorldRot = true;
                    if (Dir.GetInputValue != Vector3.zero) instr.rot = Quaternion.LookRotation(Dir.GetInputValue);
