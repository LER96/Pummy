        ...
        public override void CheckRuleOn(FieldModification mod, ref SpawnData spawn, FieldSetup preset, FieldCell cell, FGenGraph<FieldCell, FGenPoint> grid, Vector3? restrictDirection = null)
        {
...
#endif
                    thisMeasurePos = spawn.GetPosWithFullOffset(true, false);
                    //return;
                }
                else
                {
            ...
            else if (DistanceSource == ESR_Origin.RendererCenter)
            {
                ...
                else
                {
                    ...
                    if (thisRend)
                    {
                        thisMeasurePos = spawn.GetPosWithFullOffset(true, false) + spawn.GetRotationOffset() * Vector3.Scale(spawn.Prefab.transform.localScale, spawn.Prefab.transform.InverseTransformPoint(thisRend.bounds.center));
                    }
                    else
                        thisMeasurePos = spawn.GetPosWithFullOffset(true, false);
            ...
            for (int s = 0; s < spawns.Count; s++)
            {
                ...
                spawnPos = spawns[s].GetPosWithFullOffset(true, false);
                ...
                else if (DistanceSource == ESR_Origin.BoundsCenter)
                {
                    ...
                    {
                        oSpawnMeasurePos = spawns[s].GetPosWithFullOffset(true, false) + spawns[s].GetRotationOffset() * Vector3.Scale(spawns[s].Prefab.transform.localScale, spawns[s].GetRotationOffset() * spawns[s].PreviewMesh.bounds.center);
                ...
                else //if (DistanceSource == ESR_Origin.ColliderCenter)
                {
                    ...
                    if (thisRend == null)
                    {
                        ...
                        {
                            oSpawnMeasurePos = spawns[s].GetPosWithFullOffset(true, false) + spawns[s].GetRotationOffset() * Vector3.Scale(spawns[s].Prefab.transform.localScale, spawns[s].Prefab.transform.InverseTransformPoint(otherSpawnRend.bounds.center));
                            distance = Vector3.Distance(thisMeasurePos, oSpawnMeasurePos);
                        }
                    }
