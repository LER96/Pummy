...
namespace FIMSpace.Generating.Rules.QuickSolutions
{
    public class SR_WallPlacer : SpawnRuleBase, ISpawnProcedureType
    {
        ...
        [HideInInspector][Tooltip("Not removing spawns completely, just not generating model on scene, but other rules will see 'ghosted' spawns, which can be helpful for spawning props on walls with 'Get Coordinates' node")]
        ...
        public override void NodeHeader()
        {
            ...
            else
            {
                ...
                GUIIgnore[3] = "DontCheckAdditionals";
                GUIIgnore[4] = "";
            }
        }
#endif
                        ...
                        if (AutoRemoveOverlaps)
                        {
                            ...
                            if (FGenerators.CheckIfExist_NOTNULL(toRemove))
                                if (toRemove.OwnerMod == spawn.OwnerMod)
                                {
                                    ...
                                    toRemove.AddCustomStigma(SEGM);
                                        //cell.RemoveSpawnFromCell(toRemove);
                                }

                            rr = Quaternion.LookRotation(cell2.Pos - cell1.Pos);
        ...
        private void RemoveSEGMStigmedOutOfCell(FieldCell cell, SpawnData spawn)
        {
            // Removing stigmed segments out of same cell
            var stigmed = GetSpawnsWithStigmaOutOfCell(cell, SEGM);

            for (int st = 0; st < stigmed.Count; st++)
            {
                if (FGenerators.CheckIfExist_NOTNULL(stigmed[st]))
                    if (stigmed[st].OwnerMod == spawn.OwnerMod)
                    {
                        if (!SetGhosts)
        ...
        bool ShouldContinue(FieldCell cell)
        {
            ...
            for (int i = 0; i < spawns.Count; i++) if (CheckSEGMStigma(spawns[i])) return true;
        ...
        {
            var spawns = cell.CollectSpawns();
            for (int i = 0; i < spawns.Count; i++) if (spawns[i].OwnerMod == OwnerSpawner.Parent) if (CheckSEGMStigma(spawns[i])) if (SpawnRules.IsSpawnRotated(spawns[i], Quaternion.LookRotation(dir))) return true;
            return false;
        }
