    ...
    public class SR_GetCoords : SpawnRuleBase, ISpawnProcedureType
    {
                ...
                else
                {
                    ...
                    for (int a = 0; a < allSpawns.Count; a++)
                    {
                        ...
                        if (SpawnHaveSpecifics(aSpawn, GetFromTagged, CheckMode))
                        {
                            ...
                            aSpawn.TempPositionOffset = aSpawn.Offset + Quaternion.Euler(aSpawn.RotationOffset) * aSpawn.GetDirectionalOffsetWithMods();
                            //aSpawn.TempPositionOffset = GetUnitOffset(spawn.TempPositionOffset, OffsetMode, preset);
        ...
        public override void CellInfluence(FieldSetup preset, FieldModification mod, FieldCell cell, ref SpawnData spawn, FGenGraph<FieldCell, FGenPoint> grid, Vector3? restrictDirection = null)
        {
            ...
            for (int i = 0; i < getted.Count; i++)
            {
                ...
                SpawnData thisSpawn = spawn;

                if (i > 0)
                {
                    thisSpawn = spawn.Copy();
                ...
                //thisSpawn.Offset = GetUnitOffset(thisSpawn.Offset, OffsetMode, preset);
                ...
                //thisSpawn.DirectionalOffset += GetUnitOffset(DirectionalOffset, OffsetMode, preset);
                //thisSpawn.DirectionalOffset = GetUnitOffset(thisSpawn.DirectionalOffset, OffsetMode, preset);
        ...
        public override void OnConditionsMetAction(FieldModification mod, ref SpawnData thisSpawn, FieldSetup preset, FieldCell cell, FGenGraph<FieldCell, FGenPoint> grid)
        {
            if (getted != null)
            {
                if (RemoveTagged) if (string.IsNullOrEmpty(GetFromTagged) == false)
                    {
                        for (int i = 0; i < getted.Count; i++)
                        {
