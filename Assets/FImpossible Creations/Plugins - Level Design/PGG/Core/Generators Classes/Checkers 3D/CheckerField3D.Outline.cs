...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        public CheckerField3D GetOutlineChecker(int thickness = 1, ECheckerMeasureMode edgesShape = ECheckerMeasureMode.Rectangular, bool recalculate = false)
        {
                ...
                for (int i = 0; i < ChildPositionsCount; i++)
                {
                    ...
                    for (int x = -thickness; x <= thickness; x++)
                    {
                        for (int z = -thickness; z <= thickness; z++)
                        {
                            ...
                            var cell = outline.AddLocal(target);
        ...
        public CheckerField3D GetInlineChecker(bool invert = false, bool includeDiagonalCheck = true, bool detectOutDirections = false, bool recalculate = false)
        {
            ...
            for (int i = 0; i < ChildPositionsCount; i++)
            {
                ...
                {
                    FieldCell cell = inline.AddLocal(ChildPos(i));
