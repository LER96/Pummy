...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        public CheckerField3D GenerateCheckerConvertedToNewScale(Vector3 targetScale)
        {
            float myScale = RootScale.x;
            float newScale = targetScale.x;
            ...
            if (targetScale == RootScale)
            {
                newField = Copy();
            }
            else
            if (myScale > newScale) // Converting bigger field onto smaller cells
            {
            ...
            {
                UnityEngine.Debug.Log("my is smaller than new");
                ...
                for (int i = 0; i < ChildPositionsCount; i++)
                {
                    Vector3Int locOrigin = (GetLocalPos(i));

                    Vector3Int nPos = new Vector3Int();
                    nPos.x = Mathf.FloorToInt((float)locOrigin.x / iterSkipF);
                    nPos.y = Mathf.FloorToInt((float)locOrigin.y / iterSkipF);
                    nPos.z = Mathf.FloorToInt((float)locOrigin.z / iterSkipF);
        ...
        public bool CheckIfCollisionPossible(FieldCell originCell, Vector3 direction, CheckerField3D other, bool fromLocalDirToWorld = true)
        {
            if (fromLocalDirToWorld) direction = (RootRotation * direction).V3toV3Int();
            ...
            return otherBounds.IntersectRay(new Ray(worldPos, direction), out _);
        }
