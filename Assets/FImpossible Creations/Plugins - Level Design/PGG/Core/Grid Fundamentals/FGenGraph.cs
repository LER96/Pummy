...
namespace FIMSpace.Generating
{
    public partial class FGenGraph<T1, T2> where T1 : FGenCell, new() where T2 : FGenPoint, new()
    {
        ...
        public void AddCell(T1 oRef)
        {
            Cells.GetCell(oRef.Pos.x, oRef.Pos.y, oRef.Pos.z);
            Cells.ReplaceCell(oRef.Pos.x, oRef.Pos.y, oRef.Pos.z, oRef);
        }
        ...
        {
            Cells.ReplaceCell(pos.x, pos.y, pos.z, cell);
        }
        ...
        public void GetDistanceSquare2DList(List<T1> to, T1 from, float cellSize, Vector3Int indexRanges, float worldDistance)
        {
            Vector3 refPos = from.Pos;
