...
namespace FIMSpace.Generating
{
    public partial class FieldModification : ScriptableObject
    {
        public void ModifyGraphCell(FieldSetup preset, FieldCell cell, FGenGraph<FieldCell, FGenPoint> grid, bool makeNotDestroyable = false, Vector3? desiredDirection = null, bool isGuide = false, bool ignoreRestrictions = false)
        {
            ...
            for (int s = 0; s < Spawners.Count; s++) // Checking all mod's spawners
            {
                ...
                if (isGuide)
                {
                    ...
                    for (int i = 0; i < spawner.Parent.Spawners.Count; i++)
                    {
                        ArrangeGlobalRules(spawner.Parent.Spawners[i], _tempGlobalRulesPre, _tempGlobablRulesPost, _tempGlobablRulesOnConditions);

                        if (spawner.wasPreGeneratingPrepared) continue;

                        //UnityEngine.Debug.Log("prepare");
                        spawner.PreGenerating();
                        for (int r = 0; r < spawner.Rules.Count; r++)
                        {
                            if (spawner.Rules[r] == null) continue;
                            spawner.Rules[r].PreGenerateResetRule(grid, preset, spawner);
                        }
                    }
        ...
        public virtual void ModifyGraph(FieldSetup preset, FGenGraph<FieldCell, FGenPoint> grid, List<FieldCell> randomizedCells, List<FieldCell> randomizedCells2, FieldModification childMod = null)
        {
            if (grid != null)
            {
                ...
                for (int s = 0; s < Spawners.Count; s++)
                {
                    ...
                    ArrangeGlobalRules(Spawners[s], _tempGlobalRulesPre, _tempGlobablRulesPost, _tempGlobablRulesOnConditions);

                    if (Spawners[s].wasPreGeneratingPrepared) continue;
                    Spawners[s].PreGenerating();

                    for (int i = 0; i < Spawners[s].Rules.Count; i++)
                    {
                        if (Spawners[s].Rules[i] == null) continue;
                        Spawners[s].Rules[i].PreGenerateResetRule(grid, preset, Spawners[s]);
                    }
                }
                ...
                PostCall(grid, preset);
            }
        }


        void PostCall(FGenGraph<FieldCell, FGenPoint> grid, FieldSetup preset)
        {
        ...
        public virtual void ModifyGraph(CellsController cellsContr, FieldModification childModIfVariant = null, List<FieldCell> selectiveCells = null, bool resetRules = true)
        {
            ...
            // Arranging global rules
            //List<SpawnRuleBase> preGRules = new List<SpawnRuleBase>();
            //List<SpawnRuleBase> postGRules = new List<SpawnRuleBase>();
            //List<SpawnRuleBase> onCondGRules = new List<SpawnRuleBase>();

            //for (int s = 0; s < Spawners.Count; s++) // Preparing global rules for all spawners
            //{
            //    ArrangeGlobalRules(Spawners[s], preGRules, postGRules, onCondGRules);
            //}
