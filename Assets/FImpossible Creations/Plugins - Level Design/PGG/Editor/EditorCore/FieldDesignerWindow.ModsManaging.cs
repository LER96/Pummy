...
namespace FIMSpace.Generating
{
    public partial class FieldDesignWindow
    {
            ...
            if (foldout)
            {
                ...
                if (toDraw.Count > 0)
                    for (int i = 0; i < toDraw.Count; i++)
                    {
                        ...
                        {
                            GUI.enabled = preE;
                            ModificatorsPackEditor.DrawRenameScriptableButton(toDraw[i], "Modificator Pack");
                        }
        ...
        public static void DrawFieldVariablesList(List<FieldVariable> toDraw, GUIStyle style, string title, ref int selected, ref EDrawVarMode mode, UnityEngine.Object toDirty = null, bool selfInjections = true)
        {
            ...
            if (GUILayout.Button(new GUIContent(fold + "  " + title + " (" + toDraw.Count + ")", FGUI_Resources.Tex_Variables), EditorStyles.label, GUILayout.Height(20), GUILayout.Width(208))) { if (selected != -1) selected = -1; else selected = -2; }
