                            ...
                            if (DrawGrid && !usedPainter)
                            {
                                if (cell.Pos.y == focusOnY)
                                {
                                    ...
                                    if (ColorizePreview)
                                    {
                                        ...
                                        Handles.Label(pos, displayS, EditorStyles.centeredGreyMiniLabel);
                                    }
#else
                                    if (spawns.Count > 0)
                                        Handles.Label(pos, ((pos.x / cSize.x).ToString() + ", " + (pos.z / cSize.z).ToString()) + "\n" + ("Spawns: " + (spawns.Count).ToString()), EditorStyles.centeredGreyMiniLabel);
                                    else
                                        Handles.Label(pos, (pos.x / cSize.x).ToString() + ", " + (pos.z / cSize.z).ToString(), EditorStyles.centeredGreyMiniLabel);
#endif
